 ...
 var geocodingClient = mapboxSdk({accessToken: 'pk.eyJ1Ijoic2FpZm1zayIsImEiOiJjazh5cWR0b2UwZDZhM2RvZ3V0d2xwMXRkIn0.a3rJzudk7Rd0o8PuvgZ0kQ'});
	
	// On récupère les champs de la page
	//var myInput = document.getElementById("champ-ville");
	//console.log(myInput);
	//autocompleteInputBox(myInput);
	
	autocompleteInputBox(document.getElementById("ChmpAdresseDepart"));
	autocompleteInputBoxx(document.getElementById("ChmpAdresseArrivee"));
    ...
    function autocompleteInputBox(inp) {
        ...
        inp.addEventListener("input", function (e) {
            ...
            autocompleteSuggestionMapBoxAPI($('#ChmpAdresseDepart').val(), function (results) {
				//console.log('#myInput');
	...
	function autocompleteInputBoxx(inp) {
        var currentFocus;
        inp.addEventListener("input", function (e) {
            var a, b, i, val = this.value;
            closeAllLists();
            ...
            // suggestion list MapBox api called with callback
            autocompleteSuggestionMapBoxAPI($('#ChmpAdresseArrivee').val(), function (results) {
				//console.log('#myInput');
                results.features.forEach(function (key) {
                    b = document.createElement("DIV");
                    b.innerHTML = "<strong>" + key.place_name.substr(0, val.length) + "</strong>";
